...
const GOOGLE_REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI || `http://localhost:${PORT}/google-callback`;
...
function getGoogleAuthUrl(sessionId) {
...
function exchangeGoogleCodeForToken(code, sessionId) {
    return new Promise((resolve, reject) => {
        const postData = querystring.stringify({
            grant_type: 'authorization_code',
            code: code,
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    const tokenData = JSON.parse(data);
                    if (tokenData.access_token) {
                        saveSessionTokens(sessionId, { ...tokenData, provider: 'google' });
                        resolve(tokenData);
                    } else {
                        reject(tokenData);
                    }
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                if (res.statusCode === 200) {
                    ...
                    reject(new Error('Google profile access failed'));
                }
            });
        });
...
const server = http.createServer((req, res) => {
    ...
    if (pathname === '/google-auth') {
        ...
        res.setHeader('Set-Cookie', `google_session=${sessionId}; HttpOnly; Path=/; Max-Age=86400`);
        res.writeHead(302, { 'Location': authUrl });
        res.end();
        return;
    }
    ...
    if (pathname === '/google-status') {
        const cookies = req.headers.cookie || '';
        const sessionMatch = cookies.match(/google_session=([^;]+)/);
        const sessionId = sessionMatch ? sessionMatch[1] : null;
        const tokens = sessionId ? getSessionTokens(sessionId) : null;

        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({
            connected: !!(tokens && tokens.access_token && tokens.provider === 'google'),
            hasRefreshToken: !!(tokens && tokens.refresh_token),
            sessionId: sessionId,
            userProfile: tokens && tokens.userProfile ? tokens.userProfile : null
        }));
        return;
    }
    ...
    if (pathname === '/google-callback' && url.searchParams.has('code')) {
        const code = url.searchParams.get('code');
        const sessionId = url.searchParams.get('state');

        if (code && sessionId) {
            exchangeGoogleCodeForToken(code, sessionId)
                .then(async (tokenData) => {
                    console.log('Google token exchange successful for session:', sessionId);
                    // Fetch user profile
                    try {
                        const profile = await getGoogleUserProfile(tokenData.access_token);
                        const currentTokens = getSessionTokens(sessionId) || {};
                        saveSessionTokens(sessionId, { ...currentTokens, userProfile: profile });
                        res.writeHead(200, { 'Content-Type': 'text/html' });
                        res.end(`<script>
                            window.opener && window.opener.postMessage({type: 'google-connected', success: true, profile: ${JSON.stringify(profile)}}, '*');
                            setTimeout(() => window.close(), 1000);
                        </script><h1>Google Connected Successfully!</h1><p>Welcome ${profile.name}!</p><p>Closing window...</p>`);
                    } catch (profileError) {
                        console.log('Profile fetch failed:', profileError);
                        res.writeHead(200, { 'Content-Type': 'text/html' });
                        res.end(`<script>
                            window.opener && window.opener.postMessage({type: 'google-connected', success: true}, '*');
                            setTimeout(() => window.close(), 1000);
                        </script><h1>Google Connected Successfully!</h1><p>Closing window...</p>`);
                    }
                })
                .catch(error => {
                    console.log('Google token exchange failed:', error);
                    res.writeHead(400, { 'Content-Type': 'text/html' });
                    res.end('<h1>Google Authorization failed</h1>');
                });
        } else {
            res.writeHead(400, { 'Content-Type': 'text/html' });
            res.end('<h1>Google Authorization failed</h1>');
