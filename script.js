class SudokuGame {
    ...
    async checkGoogleStatus() {
        try {
            ...
            console.log('Google status response:', data); // Debug log

            // Check if user is actually signed in with valid session
            if (data && data.connected === true && data.sessionId && data.userProfile && data.userProfile.name) {
                // User has an active session - show avatar
                this.googleConnected = true;
                this.userProfile = data.userProfile;
                this.updateAccountButton('signed-in');
                this.updateUserInfo();
                console.log('User has active session - showing avatar');
            } else {
                // No active session - show sign up button
                this.googleConnected = false;
                this.userProfile = null;
                this.updateAccountButton('sign-up');
                this.updateUserInfo();
                console.log('No active session - showing sign up button');
            }

        } catch (error) {
            console.error('Failed to check Google status:', error);
            // On error, show sign up button
            this.googleConnected = false;
            this.userProfile = null;
            this.updateAccountButton('sign-up');
            this.updateUserInfo();
    ...
    updateAccountButton(state) {
        ...
        if (state === 'signed-in' && this.userProfile) {
            // Hide sign up button and show avatar
            if (accountBtn) {
                accountBtn.style.display = 'none';
                console.log('Hiding sign up button - showing avatar');
            }
            if (userAvatar) {
                const firstLetter = (this.userProfile.name || this.userProfile.email || 'U').charAt(0).toUpperCase();
                userAvatar.textContent = firstLetter;
                userAvatar.style.display = 'flex';
                console.log('Showing avatar with letter:', firstLetter);
            }
        } else {
            // Show sign up button and hide avatar
            if (accountBtn) {
                accountBtn.style.display = 'inline-block';
                console.log('Showing sign up button - hiding avatar');
            }
            if (userAvatar) {
    ...
    updateUserInfo() {
        // User info is now handled by the avatar
        this.updateAccountButton(this.googleConnected ? 'signed-in' : 'sign-up');
    }

    connectGoogle() {
        // Always allow sign-in attempts - don't check if already connected
        // since we're being conservative about connection status

        // Close dropdown immediately after clicking
        this.toggleAccountDropdown();

        const authWindow = window.open('/google-auth', 'google-auth',
            'width=500,height=600,scrollbars=yes,resizable=yes');

        const messageHandler = (event) => {
            if (event.data && event.data.type === 'google-connected') {
                window.removeEventListener('message', messageHandler);
                if (event.data.success) {
                    this.googleConnected = true;
                    this.userProfile = event.data.profile;
                    // Now show avatar instead of button
                    this.updateAccountButton('signed-in');
                    this.updateUserInfo();
                    this.showNotification(`Welcome ${event.data.profile.name}! You are now signed in.`);
                } else {
                    this.updateAccountButton('sign-up');
                    this.showNotification('Failed to connect to Google');
                }
            }
        };
        ...
        if (!authWindow) {
            this.showNotification('Please allow popups for this site to sign in with Google');
            return;
        }
        ...
        const checkClosed = setInterval(() => {
            if (authWindow.closed) {
                clearInterval(checkClosed);
                window.removeEventListener('message', messageHandler);
                // Keep button visible if sign-in didn't succeed
    ...
    signOut() {
        ...
        // Close dropdown after signing out
        this.toggleAccountDropdown();
        // Note: In a real implementation, you'd want to clear the session on the server side too
        console.log('User signed out - sign up button should be visible');
    }
